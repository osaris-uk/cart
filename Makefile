RUNNER=docker run -it --rm --workdir "/src" -v "$(PWD):/src" -v "$(HOME)/.composer/cache:/root/.composer/cache" cartphp /bin/bash -c

.PHONY: build composer php test

build:
	@docker build --build-arg VERSION=7.2 --tag=cartphp .
composer:
